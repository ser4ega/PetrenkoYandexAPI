...
def get_routes(from_city, to_city, date,transport_type):
    ...
    url = f"https://api.rasp.yandex.net/v3.0/search/?apikey={API_KEY}&lang=ru_RU&format=json&from={from_code}&to={to_code}&date={date}&transport_types={transport_type}&limit=3"
...
def find_routes():
    ...
    # сперва найдем все рейсыф из стартового города к остальным городам
    routes_datas.append(get_routes(from_city, to_city, date,transport_type))
    for i in a:
        trans_city_1 = request.form.get("city-"+str(i))
        routes_datas.append(get_routes(from_city, trans_city_1, date,transport_type))
    # теперь добавим к списку маршруты из промежуточных городов в конечный
        routes_datas.append(get_routes(trans_city_1, to_city, date,transport_type))
    ...
    for i, j in [(i,j) for i in a for j in a]:
            if i!=j:
                ...
                routes_datas.append(get_routes(trans_city_1, trans_city_2, date,transport_type))
                routes_datas.append(get_routes(trans_city_1, trans_city_2, date,transport_type)) 
    return render_template("routes.html", routes_data=routes_datas)
